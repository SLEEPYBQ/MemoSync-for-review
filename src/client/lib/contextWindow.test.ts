...
describe("deriveLatestContextWindowSnapshot", () => {
  ...
  test("does not reuse a pre-compaction usage snapshot after the context resets", () => {
    const messages = [
      entry({ kind: "context_window_updated", usage: { usedTokens: 199_500, maxTokens: 200_000, compactsAutomatically: true } }, 1),
      entry({ kind: "compact_boundary" }, 2),
    ]

    expect(deriveLatestContextWindowSnapshot(messages)).toBeNull()
  })
})
