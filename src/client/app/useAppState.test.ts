...
import {
  ...
  shouldRefreshStaleChatSnapshot,
...
describe("runChatStartAction", () => {
  test("returns a visible failure result while preserving the existing global error report", async () => {
    const reported: string[] = []

    const result = await runChatStartAction(
      async () => {
        throw new Error("chat.create failed")
      },
      (message) => reported.push(message)
    )

    expect(result).toEqual({ ok: false, error: "chat.create failed" })
    expect(reported).toEqual(["chat.create failed"])
  })
})
...
describe("shouldRefreshStaleChatSnapshot", () => {
  const stale = {
    activeChatId: "chat-1",
    isProcessing: true,
  ...
  test("refreshes only a connected processing chat after twenty quiet seconds", () => {
    expect(shouldRefreshStaleChatSnapshot(stale)).toBe(true)
    expect(shouldRefreshStaleChatSnapshot({ ...stale, now: 29_999 })).toBe(false)
    expect(shouldRefreshStaleChatSnapshot({ ...stale, isProcessing: false })).toBe(false)
    expect(shouldRefreshStaleChatSnapshot({ ...stale, activeChatId: null })).toBe(false)
    expect(shouldRefreshStaleChatSnapshot({ ...stale, connectionStatus: "disconnected" })).toBe(false)
  })

  test("a received snapshot or a recent refresh restarts the quiet window", () => {
    expect(shouldRefreshStaleChatSnapshot({ ...stale, lastSnapshotAt: 20_001 })).toBe(false)
    expect(shouldRefreshStaleChatSnapshot({ ...stale, lastRefreshAt: 20_001 })).toBe(false)
  })
})
...
describe("resolveComposeIntent", () => {
  test("prefers an explicit local path over every other target", () => {
    expect(
      resolveComposeIntent({
        explicitLocalPath: "/tmp/picked-project",
        selectedProjectId: "project-selected",
        sidebarProjectId: "project-sidebar",
        fallbackLocalProjectPath: "/tmp/project",
      })
    ).toEqual({ kind: "local_path", localPath: "/tmp/picked-project" })
  })
