...
describe("rightSidebarStore", () => {
  ...
  test("defaults to the memory panel (redesign 2026-08-07)", () => {
    const visibility = useRightSidebarStore.getState().projects[PROJECT_ID] ?? getDefaultRightSidebarVisibilityState()
    expect(visibility.rightPanel).toBe("memory")
    expect(visibility.openInitiator).toBe("system")
    expect(useRightSidebarStore.getState().size).toBe(DEFAULT_RIGHT_SIDEBAR_SIZE)
  })

  test("distinguishes the automatic memory-panel mount from participant close and reopen", () => {
    expect(getDefaultRightSidebarVisibilityState()).toEqual({
      rightPanel: "memory",
      openInitiator: "system",
    })
    useRightSidebarStore.getState().togglePanel(PROJECT_ID, "memory")
    expect(useRightSidebarStore.getState().projects[PROJECT_ID]).toMatchObject({ rightPanel: "hidden" })
    useRightSidebarStore.getState().togglePanel(PROJECT_ID, "memory")
    expect(useRightSidebarStore.getState().projects[PROJECT_ID]).toEqual({
      rightPanel: "memory",
      openInitiator: "participant",
    })
  })

  test("treats a participant-opened panel restored after reload as system-opened", () => {
    useRightSidebarStore.getState().togglePanel(PROJECT_ID, "git")
    useRightSidebarStore.getState().setSize(537)
    expect(useRightSidebarStore.getState().projects[PROJECT_ID]?.openInitiator).toBe("participant")

    const restored = mergeRightSidebarStore(
      JSON.parse(JSON.stringify(useRightSidebarStore.getState())),
      useRightSidebarStore.getState(),
    )

    expect(restored.projects[PROJECT_ID]).toEqual({
      rightPanel: "git",
      openInitiator: "system",
    })
    expect(restored.size).toBe(537)
  })
