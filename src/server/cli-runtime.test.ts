...
describe("runCli", () => {
  test("never checks or installs the upstream Kanna package for the MemoSync fork", async () => {
    ...
    expect(calls.startServer[0]?.update).toBeUndefined()
  })
  ...
  test("starts normally when no newer version exists", async () => {
    const { calls, deps } = createDeps()
    process.env.MEMOSYNC_RUNTIME_PROFILE = "prod"
    ...
    expect(calls.log).toContain("[memosync] data dir: ~/.memosync/data")
  })
  ...
  test("logs the dev data dir when the dev runtime profile is active", async () => {
    ...
    expect(calls.log).toContain("[memosync] data dir: ~/.memosync-dev/data")
  })
  ...
  test("fails fast on unsupported Bun versions", async () => {
    ...
    expect(calls.startServer).toEqual([])
    expect(calls.warn).toContain("[memosync] Bun 1.3.5+ is required for the embedded terminal. Current Bun: 1.3.1")
  })
  ...
  test("logs share setup progress from the default tunnel helper", async () => {
    ...
    deps.startShareTunnel = async (_localUrl) => {
      deps.log("[memosync] installing cloudflared binary")
      installLogged = true
      return {
        publicUrl: "https://kanna.trycloudflare.com",
        stop: () => {},
    ...
    expect(calls.log).toContain("[memosync] installing cloudflared binary")
  })
  ...
  test("fails cleanly when share tunnel startup fails", async () => {
    ...
    expect(calls.warn).toContain("[memosync] failed to start Cloudflare share tunnel")
    expect(calls.warn).toContain("[memosync] cloudflared unavailable")
  })
  ...
  test("keeps running when a named tunnel starts without a detected hostname", async () => {
    ...
    expect(calls.warn).toContain("[memosync] named tunnel started but no public hostname was detected")
    expect(calls.warn).toContain("[memosync] use the hostname configured for the provided Cloudflare tunnel token")
    expect(calls.log).toContain("Local URL:")
    expect(calls.log).toContain("http://localhost:3210")
    expect(calls.renderShareQr).toEqual([])
  })

  test("ignores a newer upstream version", async () => {
    const { calls, deps } = createDeps({
      fetchLatestVersion: async (packageName) => {
        calls.fetchLatestVersion.push(packageName)
        return "0.4.0"
  ...
  test("does not invoke the upstream installer", async () => {
    const { calls, deps } = createDeps({
      fetchLatestVersion: async (packageName) => {
        calls.fetchLatestVersion.push(packageName)
        return "0.4.0"
  ...
  test("does not contact the upstream registry", async () => {
    const { calls, deps } = createDeps({
      fetchLatestVersion: async (packageName) => {
        calls.fetchLatestVersion.push(packageName)
